...
class RequestCompleted extends Mailable
{
    ...
    public function __construct($insdetails,$companydetails,$requestdetails,$subject,$type)
    {
        ...
        {
            $this->body = $options->get_option('inspector_request_completed_message');
        }
        else if($type=='companyassign')
        {
            $this->body = $options->get_option('company_request_completed_message');
        }
        else if($type=='applicantassign')
        {
            $this->body = $options->get_option('applicant_request_completed_message');
        }
        if($type=='inspectorassign')
        {
        ...
        else
        {
            ...
            $this->body= str_replace('[inspector_name]',$insdetails->name,$this->body);
            $this->body= str_replace('[company_name]',$requestdetails->company_name,$this->body);
            $this->body= str_replace('[company_location]',$companydetails->company_address,$this->body);
